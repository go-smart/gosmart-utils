...
problemname = "microsulis-probe-sensitivity"
...
EM_parameters.Pin = args.power # power in
EM_parameters.r_1 = 0.00016 # inner conductor
EM_parameters.r_2 = 0.00047 # outer conductor
...
with open('tests/microsulis-materials.yml', 'r') as f:
    materials = yaml.load(f)
with open('tests/microsulis-regions.yml', 'r') as f:
    regions = yaml.load(f)

EM_parameters.eps_r_by_subdomain = [-1] + [0] * len(regions)
EM_parameters.sigma_by_subdomain = [-1] + [0] * len(regions)
EM_parameters.mu_r_by_subdomain =  [-1] + [0] * len(regions)
for n, reg in regions.items():
    i = reg['region']
    EM_parameters.eps_r_by_subdomain[i] = materials[reg['material']]['eps_r']
    EM_parameters.sigma_by_subdomain[i] = materials[reg['material']]['sigma']
    EM_parameters.mu_r_by_subdomain[i] = materials[reg['material']]['mu_r']

thermal_parameters.restrict_th_mesh = regions['tissue']['region']  # region to compute thermal solution in
...
files = [File("%s-%d/U.pvd" % (problemname, int(EM_parameters.Pin))),
         File("%s-%d/E_z.pvd" % (problemname, int(EM_parameters.Pin))),
         File("%s-%d/E_r.pvd" % (problemname, int(EM_parameters.Pin))),
         File("%s-%d/Q.pvd" % (problemname, int(EM_parameters.Pin))),
         File("%s-%d/sigma.pvd" % (problemname, int(EM_parameters.Pin))),
         File("%s-%d/Q_over_sigma.pvd" % (problemname, int(EM_parameters.Pin)))]


# compute SAR
thermal_parameters.T0 = Constant(0.)
U, Q, E_r, E_z = compute_SAR_nl(problemname, mesh, interior, boundaries, EM_parameters, thermal_parameters.T0, thermal_parameters, files)
...
thermal_parameters.tol = 1e-4 # steady state tolerance
thermal_parameters.relaxation = 0.1 # steady state relaxation coefficient
...
# transient calc
T = compute_enthalpy_nl(mesh, interior, boundaries, problemname, dt, tmax, dt_min, dt_max, t_out, thermal_parameters, EM_parameters, files)
